...
- 省级 GeoJSON 面图层尚未真正启用；
- 去过省份 / 未去过省份还没有在地图上直接以省份填色区分；
- 原因是缺少 `public/data/china-provinces.geojson`；
- 当前 `JourneyMap.astro` 已有 GeoJSON 接入和优雅降级逻辑，但没有真实边界文件时只显示城市 marker。
...
| 首页 | `/` | 已完成总目录化和视觉基底改造 |
| 关于我 | `/about` | 稳定 |
| 旅程地图 | `/journeys` | 城市 marker、层级目录和联动可用；省级面填色待 GeoJSON |
